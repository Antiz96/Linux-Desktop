...
```bash
sudo pacman -S ccid discord distrobox docker fastfetch firefox firejail htop keepassxc mlocate mpv noto-fonts-emoji ntfs-3g powerline-fonts rsync steam systray-x thunderbird tmux ttf-font-awesome virt-viewer wl-clipboard wofi xorg-xwayland yubico-piv-tool zathura zathura-pdf-poppler #Main packages from Arch repos
