...
```bash
sudo pacman -S capitaine-cursors ccid discord distrobox docker fastfetch firefox firefoxpwa firejail htop keepassxc mpv mumble noto-fonts-emoji orchis-theme plocate powerline-fonts protonmail-bridge rsync speedcrunch steam systray-x tela-circle-icon-theme-blue thunderbird tmux otf-font-awesome vim-devicons vim-nerdtree virt-viewer wireguard-tools wl-clip-persist xorg-xwayland yubico-piv-tool zathura zathura-pdf-poppler
