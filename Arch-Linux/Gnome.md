...
```bash
sudo pacman -S discord distrobox docker firefox glow hexchat htop keepassxc mlocate neofetch noto-fonts-emoji ntfs-3g pasystray steam systray-x thunderbird tmux virt-viewer vlc xclip zathura zathura-pdf-poppler #Main packages from Arch repos
yay -S arch-update gnome-browser-connector gnome-terminal-transparency onlyoffice-bin protonmail-bridge-bin spotify timeshift ventoy-bin zaman #Main packages from the AUR
