...
```bash
sudo pacman -S discord distrobox docker firefox glow hexchat htop keepassxc mlocate neofetch noto-fonts-emoji ntfs-3g pasystray rofi steam systray-x thunderbird tmux virt-viewer vlc xclip xorg-xhost zathura zathura-pdf-poppler #Main packages from Arch repos
yay -S arch-update onlyoffice-bin protonmail-bridge-bin spotify timeshift ventoy-bin zaman #Main packages from the AUR
