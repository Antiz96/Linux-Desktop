...
```bash
sudo pacman -S abuild atools capitaine-cursors ccid discord distrobox docker fastfetch firefox firefoxpwa firejail htop keepassxc mpv mumble noto-fonts-emoji orchis-theme plocate powerline-fonts protonmail-bridge rofi rsync speedcrunch steam systray-x tela-circle-icon-theme-blue thunderbird tmux otf-font-awesome vim-devicons vim-nerdtree virt-viewer wireguard-tools xclip xorg-xhost yubico-piv-tool zathura zathura-pdf-poppler
paru -S arch-update nerdtree-git-plugin-git onlyoffice-bin pa-applet-git ventoy-bin zaman
sudo pacman -S --asdeps gnome-keyring gnu-free-fonts systemd-resolvconf ttf-dejavu ttf-nerd-fonts-symbols xdg-utils # Optional dependencies I need for the above packages
