...
```bash
sudo pacman -S capitaine-cursors ccid discord distrobox docker fastfetch firefox firejail htop keepassxc mpv mumble noto-fonts-emoji orchis-theme plocate powerline-fonts protonmail-bridge rofi rsync speedcrunch steam systray-x tela-circle-icon-theme-blue thunderbird tmux ttf-font-awesome vim-auto-pairs vim-devicons vim-nerdtree virt-viewer xclip xorg-xhost yubico-piv-tool zathura zathura-pdf-poppler
