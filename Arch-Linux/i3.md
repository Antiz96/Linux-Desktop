...
```bash
sudo pacman -S abuild atools-go capitaine-cursors ccid discord distrobox fastfetch firefox firefoxpwa firejail htop keepassxc mpv mumble noto-fonts-emoji orchis-theme plocate podman powerline-fonts protonmail-bridge rofi rsync speedcrunch steam systray-x tela-circle-icon-theme-blue thunderbird tmux otf-font-awesome vim-devicons vim-nerdtree wireguard-tools xclip xorg-xhost yubico-piv-tool zathura zathura-pdf-poppler
